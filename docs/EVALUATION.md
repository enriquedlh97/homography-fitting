...
uv run python -m banner_pipeline.eval \
    ...
    [--with-ai-review]                                # opt-in Anthropic API rubric (cost ~$0.10-0.30/run)
```
...
experiments/<run>/
  ...
  eval/
    ...
    ai_review/                            # only with --with-ai-review (or written manually)
      back_banners.{json,md}
      left_logo.{json,md}
      floor_logo.{json,md}
      walkover.{json,md}
      full.{json,md}
...
### Layer 1: PNG strips (always produced)

Every region gets `crops_strip.png` (6 evenly-spaced frames, 3× upscaled) and three `consecutive_frames_*.png` (8 contiguous frames each, at 15% / 50% / 85% of the clip). The walkover region gets a denser per-window strip. PNG strips are the floor of "scan in seconds" review.
...
### Layer 3: AI visual inspection (vision-LLM rubric)

Two paths, same structured rubric:

**Manual (free):** ask the conversation Claude (or any vision-capable assistant) to "review run X." It Reads the PNGs from `eval/<region>/` and `eval/walkover/`, fills out the rubric, writes `eval/ai_review/<region>.md`.

**Automated (`--with-ai-review`, ~$0.10–0.30/run):** `src/banner_pipeline/eval/ai_review.py` calls the Anthropic API (Claude Opus 4.7 with vision, prompt caching mandatory). Inputs are the same PNGs. Outputs strict-JSON validated by Pydantic at `eval/ai_review/<region>.json` plus prose at `eval/ai_review/<region>.md`.
...
Mapped from the legacy 10-point visual QA checklist (in `docs/evaluation-protocol.md`) — preserved as rubric dimensions so a sub-agent's review is comparable to a human's.

AI review is **warning-only** at first; never gates exit code. After ~10 graded runs we calibrate which rubric dimensions become gates.
...
2. Run `python -m banner_pipeline.eval --experiment <dir> --reference auto`.
...
4. (Optional) Re-run with `--with-ai-review` if the agent's task is checkpoint-grade or close-to-ship; otherwise skip to save API cost.
...
- LPIPS / DreamSim / VGG-perceptual learned metrics. The AI rubric covers perceptual realism qualitatively.
