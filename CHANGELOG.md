...
- **Visual review override:** rejected the rubric-best Phase 3 variant (P3-A38/e2). Layered shadow synthesis + `erase_text=true` + tightened banner edges produced visible regressions on direct viewing despite scoring 5/5 on the structured visual rubric. P3-A1 keeps V68's compositor and only adds dynamic homography. See `docs/FINAL_REPORT.md` §6.5.
...
- **P3-A38/e2, rubric-best variant (rejected on visual review).** Recipe = P3-A1 + shadow_strength=0.6 + erase_text=true + obj_4 padding=0. Scored 5/5 on the rubric for the user-flagged artifacts. Visual review on 2026-05-06 rejected this in favor of the simpler P3-A1.
