...
class InpaintCompositor(Compositor):
    ...
    def __init__(self) -> None:
        ...
        # Per-obj_id EMA cache of the fill colour used by `median_fill` /
        # `gradient_fill`. Stabilises the visible background under the new
        # logo (transparent regions of the overlay show this fill).
        self._fill_color_ema: dict[int, np.ndarray] = {}
    ...
    def composite(
        ...
        **kwargs,
        ...
        with Timer("inpaint.inpaint"):
            if do_inpaint and mask_roi is not None:
                ...
                if inpaint_method == "black_fill":
                    ...
                    # Optional EMA across frames (per obj_id) eliminates
                    # residual jitter so the visible bg under the new logo
                    # stays consistent.
                    border = (
                        cv2.dilate(
                            mask_u8_roi,
                    ...
                    if np.any(border > 0):
                        ...
                        ema_a = float(kwargs.get("fill_color_ema_alpha", 0.1))
                        obj_id = kwargs.get("obj_id")
                        if obj_id is not None and 0.0 <= ema_a < 1.0:
                            prev = self._fill_color_ema.get(int(obj_id))
                            smoothed = (
                                sampled if prev is None
                                else ema_a * sampled + (1.0 - ema_a) * prev
                            )
                            self._fill_color_ema[int(obj_id)] = smoothed
                            fill_color = smoothed.astype(np.uint8)
                        else:
                            fill_color = sampled.astype(np.uint8)

                        # Build the actual fill region: union of dilated SAM
                        # mask AND the (optionally padded) quad polygon.
                        # The quad-padding catches the anti-aliasing halo of
                        # the original logo that SAM under-segmented — i.e.
                        # the coloured ring that otherwise remains around
                        # the new logo.
                        quad_pad = int(kwargs.get("quad_pad_px", 0))
                        c_roi = corners_roi.astype(np.float32).copy()
                        if quad_pad > 0:
                            center = c_roi.mean(axis=0)
                            for i in range(4):
                                direction = c_roi[i] - center
                        ...
                        cv2.fillPoly(
                            quad_mask,
                            [c_roi.astype(np.int32).reshape((-1, 1, 2))],
                            255,
                        )
                        fill_region = np.maximum(mask_u8_roi, quad_mask)

                        inpainted_roi[fill_region > 0] = fill_color
        ...
        # For fill methods that extend the cleaned area beyond the SAM mask
        # (black_fill, median_fill, gradient_fill), expand the logo's quad
        # by the same `quad_pad_px` so the new logo physically covers the
        # padded fill region — otherwise the padding ring would show as a
        # solid colour border around the new logo.
        logo_corners = corners.copy()
        logo_corners_roi = corners_roi.copy()
        if inpaint_method in ("black_fill", "median_fill", "gradient_fill"):
            quad_pad = int(kwargs.get("quad_pad_px", 0))
            if quad_pad > 0:
